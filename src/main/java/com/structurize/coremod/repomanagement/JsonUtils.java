...
public class JsonUtils
{
     ...
     */
    protected static Style parseToStyle(final String json, final String id, final ResourceLocation origin, final String author)
    {
        try
        {
            ...
            final Style newStyle = new Style(
                    id,
                    parsedJson.get(JSON_NAME).getAsString(),
                    parsedJson.get(JSON_DESCRIPT).getAsString(),
                    getOptionalStringTag(JSON_IMAGE, parsedJson),
                    origin,
                    getOptionalStringTag(JSON_JOKE, parsedJson),
                    author,
                    getOptionalStringTag(JSON_BASE, parsedJson),
            ...
            for(int i = 0; i < jsonHeadsArray.size(); i++)
            {
                ...
                final String newHeadName = parsedJson.get(JSON_NAMES) != null ? parsedJson.get(JSON_NAMES).getAsJsonArray().get(i).getAsString() : "";
                ...
                for(int j = 0; j < jsonTypesArray.size(); j++)
                {
                    ...
                    final String newTypeName = head.get(JSON_NAMES) != null ? head.get(JSON_NAMES).getAsJsonArray().get(j).getAsString() : "";
                    ...
                    for(int k = 0; k < jsonFilesArray.size(); k++)
                    {
                        ...
                        final String newFileName = type.get(JSON_NAMES) != null ? type.get(JSON_NAMES).getAsJsonArray().get(k).getAsString() : "";
            ...
            {
                final String isAnyHead = parsedJson.get(JSON_ANY) != null ? JSON_ANY : jsonHeadsArray.get(i).getAsString();
                final JsonObject head = parsedJson.get(isAnyHead).getAsJsonObject();
                ...
                {
                    final String jsonTypeId = jsonTypesArray.get(j).getAsString().replace("$head ", jsonHeadsArray.get(i).getAsString());
                    final String isAnyType = head.get(JSON_ANY) != null ? JSON_ANY : jsonTypeId;
                    final JsonObject type = head.get(isAnyType).getAsJsonObject();
                    final DefaultType newDefaultType = new DefaultType(newStyleType.get(j), parsedJson.get(JSON_FILE_EXT).getAsString());
                    final List<String> registryFiles = new ArrayList<String>();

                    // loop for files
                    if (type.get(JSON_REGISTRY) != null && !type.get(JSON_REGISTRY).isJsonArray() && type.get(JSON_REGISTRY).getAsString() == "$files")
                    {
                    ...
                    if (head.get(JSON_REGISTRY) != null && !head.get(JSON_REGISTRY).isJsonArray() && head.get(JSON_REGISTRY).getAsString() == "$types")
                    {
                ...
                if (parsedJson.get(JSON_REGISTRY) != null && !parsedJson.get(JSON_REGISTRY).isJsonArray() && parsedJson.get(JSON_REGISTRY).getAsString() == "$heads")
                {
    ...
    /**
     * Parses given JSON to return a new List of Strings with URL suffixes from given attibute
     * 
    ...
    {
        return new Gson().fromJson(new JsonParser().parse(json).getAsJsonObject().get(jsonAttribute), new TypeToken<T>() {}.getType());
    }
     ...
     * @param extendsT is type of expected json element
     */
    public static <T> T parseSecondLayerAttribute(final String json, final String jsonFirstAttribute, final String jsonSecondAttribute, T extendsT)
    {
        return new Gson().fromJson(new JsonParser().parse(json).getAsJsonObject().get(jsonFirstAttribute).getAsJsonObject().get(jsonSecondAttribute), new TypeToken<T>() {}.getType());
    }
    ...
    protected static List<Tuple<String, String>> parseFirstLayerObjectArrayToTuple(final String json, final String jsonFirstAttribute, final String jsonTupleFirst, final String jsonTupleSecond)
    {
        final List<Tuple<String, String>> result = new ArrayList<Tuple<String, String>>();
        final JsonArray jsonArray = new JsonParser().parse(json).getAsJsonObject().get(jsonFirstAttribute).getAsJsonArray();

        jsonArray.forEach(jsonElement -> {
            final JsonObject jsonObject = jsonElement.getAsJsonObject();
            result.add(new Tuple<String, String>(jsonObject.get(jsonTupleFirst).getAsString(), jsonObject.get(jsonTupleSecond).getAsString()));
        });
