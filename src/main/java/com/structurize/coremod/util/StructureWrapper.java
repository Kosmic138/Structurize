...
public class StructureWrapper {
     ...
     */
    public static void unloadStructure(@NotNull final World world, @NotNull final BlockPos pos,
            @NotNull final String first, final int rotation, @NotNull final Mirror mirror) {
        @NotNull
        final StructureWrapper structureWrapper = new StructureWrapper(world, first);
        ...
        for (int j = 0; j < this.structure.getHeight(); j++) {
            for (int k = 0; k < this.structure.getLength(); k++) {
                for (int i = 0; i < this.structure.getWidth(); i++) {
                    @NotNull
                    final BlockPos localPos = new BlockPos(i, j, k);
    ...
    /**
     * Load a structure into this world and place it in the right position and
     * rotation.
     * 
     ...
     */
    public static void loadAndPlaceShapeWithRotation(final WorldServer worldObj, final Template template,
        ...
        try {
            ...
            @NotNull
            final StructureWrapper structureWrapper = new StructureWrapper(worldObj, proxy,
                    "shape" + player.getName() + ".nbt");
            structureWrapper.position = pos;
    ...
    public static void loadAndPlaceStructureWithRotation(final World worldObj, @NotNull final String name,
            @NotNull final BlockPos pos, final int rotations, @NotNull final Mirror mirror, final boolean complete,
            final EntityPlayerMP player) {
        try {
            @NotNull
            final StructureWrapper structureWrapper = new StructureWrapper(worldObj, name);
            structureWrapper.position = pos;
            structureWrapper.rotate(rotations, worldObj, pos, mirror);
     ...
     */
    public void rotate(final int times, @NotNull final World world, @NotNull final BlockPos rotatePos,
     ...
     */
    public void setupStructurePlacement(@NotNull final BlockPos pos, final boolean complete,
            final EntityPlayerMP player) {
        this.setLocalPosition(pos);
    ...
    public BlockPos placeStructure(final World world, final ChangeStorage storage, final BlockPos inputPos,
        ...
        @NotNull
        final List<BlockPos> delayedBlocks = new ArrayList<>();
        final BlockPos endPos = new BlockPos(this.structure.getWidth(), this.structure.getHeight(),
                this.structure.getLength());
        ...
        for (int y = currentPos.getY(); y < endPos.getY(); y++) {
            for (int x = currentPos.getX(); x < endPos.getX(); x++) {
                for (int z = currentPos.getZ(); z < endPos.getZ(); z++) {
                    @NotNull
                    final BlockPos localPos = new BlockPos(x, y, z);
                    ...
                    if ((localBlock == ModBlocks.blockSubstitution && !complete)
                            || localBlock instanceof IAnchorBlock) {
                        continue;
                    }
                    count++;
                    ...
                    if (localState.getMaterial().isSolid()) {
                        this.handleBlockPlacement(world, worldPos, localState, complete,
                                this.structure.getBlockInfo(localPos).tileentityData);
                    } else {
     ...
     * @param world         the world.
     */
    private void handleDelayedBlocks(final List<BlockPos> delayedBlocks, final ChangeStorage storage,
            final World world) {
        for (@NotNull
        final BlockPos coords : delayedBlocks) {
            ...
            this.handleBlockPlacement(world, newWorldPos, localState, this.complete,
                    this.structure.getBlockInfo(coords) == null ? null
                            : this.structure.getBlockInfo(coords).tileentityData);
        }
    }
    ...
    /**
     * This method handles the block placement. When we extract this into another
     * mod, we have to override the method.
     *
     * @param world          the world.
     ...
     */
    public void handleBlockPlacement(final World world, final BlockPos pos, final IBlockState localState,
            final boolean complete, final NBTTagCompound tileEntityData) {
    ...
    public boolean isStructureBlockEqualWorldBlock() {
        ...
        // had this problem in a super flat world, causes builder to sit doing nothing
        ...
        for (int j = 0; j < this.structure.getHeight(); j++) {
            for (int k = 0; k < this.structure.getLength(); k++) {
                for (int i = 0; i < this.structure.getWidth(); i++) {
                    ...
                    if (worldPos.getY() <= pos.getY()
                            && !this.world.getBlockState(worldPos.down()).getMaterial().isSolid()) {
                        return false;
                    }
    ...
    public boolean decrementBlock() {
        if (this.progressPos.equals(NULL_POS)) {
            this.progressPos.setPos(this.structure.getWidth(), this.structure.getHeight() - 1,
                    this.structure.getLength() - 1);
        }

        this.progressPos.setPos(this.progressPos.getX() - 1, this.progressPos.getY(), this.progressPos.getZ());
        if (this.progressPos.getX() == -1) {
            this.progressPos.setPos(this.structure.getWidth() - 1, this.progressPos.getY(),
    ...
    /**
     * @return The StructureProxy that houses all the info about what is stored in a
     *         structure.
     */
