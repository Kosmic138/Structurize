...
import com.structurize.blockout.Loader;
...
import com.structurize.coremod.repomanagement.repostructure.DefaultStyle;
...
public class WindowManageStyles extends OverlayView implements ButtonHandler
{
    ...
    private static       int    MAX_NUM_OF_TABS = 4; // for some reason it can't be final eventhough the init only is in the constructor
    ...
    private void setDisplayStyle(Style style)
    {
        ...
        findPaneOfTypeByID(JSON_BASE, Text.class).setTextContent((style instanceof DefaultStyle) ? "Default style" : (style.getBase().isEmpty()) ? "" : "Is based on " + styles.getStyleById(style.getBase()).getName() +  " style");
        ...
        findPaneOfTypeByID(JSON_IMAGE, Image.class).setImage(style.getImage().isEmpty() ? "structurize:textures/gui/styles_sample_image.png" : "local:" + style.getImage());
    ...
    private void adjustTabsBy(int adjust)
    {
        ...
        int newIndex = stylesList.indexOf(styles.getStyleByName(findPaneOfTypeByID(JSON_NAME + "0", Button.class).getLabel())) + adjust;
