/*
 ...
 Colonies have an integer ID, and contain all data for a management.  They are mirrored to the client as ColonyView objects.

 Buildings in a management are represented via Building subclass objects, and mirrored to clients as matching Building.View
 ...
 Colonies maintain a set of Permissions, which are the relationship of individual players to a management, and
 (possibly eventually) the privileges available to each relationship type.

 Tasks to be performed in a management are centralized as WorkOrders within the WorkManager of a management.  This allows
 ...
 Data for a citizen / worker / building / management should be exist in the locations as follows:
 ...
 * Colony = Data relevant to the management as a whole

 ## Classes

 ### Manager
 The Manager class is a singleton, implemented via only static members and methods.  It is usable from both server and client.

 On the server, the Manager maintains ownership of all colonies, and provides lookup access to them by ID.

 On the client, the Manager maintains the known ColonyViews and provides lookup access by ID.

 ### Colony and ColonyView classes
 Colony is the server-side representation of a management, and maintains all data for a given a management.
 ...
 Building classes are the server-side representation of buildings in a management, maintaining state for a given building.
 ...
 ### Permissions
 Permissions management for relationship of plauyers to the management.

 Mirrored to clients as Permissions.View.

 ### WorkManager and WorkOrder subclasses
 Manages a list of tasks for the management, providing a central system for multiple components to coordinate.
