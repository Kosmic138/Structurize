...
public class RemoveEntityMessage extends AbstractMessage<RemoveEntityMessage, IMessage> {
     ...
     */
    public RemoveEntityMessage(@NotNull final BlockPos pos1, @NotNull final BlockPos pos2,
    ...
    public void messageOnServerThread(final RemoveEntityMessage message, final EntityPlayerMP player) {
        ...
        for (int x = Math.min(message.from.getX(), message.to.getX()); x <= Math.max(message.from.getX(),
                message.to.getX()); x++) {
            for (int y = Math.min(message.from.getY(), message.to.getY()); y <= Math.max(message.from.getY(),
                    message.to.getY()); y++) {
                for (int z = Math.min(message.from.getZ(), message.to.getZ()); z <= Math.max(message.from.getZ(),
                        message.to.getZ()); z++) {
