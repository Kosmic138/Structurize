...
public class FileManager {
    ...
    /**
     * Called from init ForgeEvent, loads all information about styles in
     * Configurations.repositoriesUrls
     */
    public static void loadRepositories(final String[] repositories) {
        ...
        for (String url : repositories) {
            ...
            if (resourceLoc.getNamespace().equals(GITHUB_REPO_PREFIX)) {
                ...
                JsonUtils.parseFirstLayerAttribute(NetUtils.httpsGet(githubUrl + GITHUB_URL_MASTER_BRANCHES_JSON),
                        JSON_BRANCHES, new ArrayList<String>()).forEach(branch -> {

                            // get author
                            final String author = JsonUtils.parseFirstLayerAttribute(
                                    NetUtils.httpsGet(githubUrl + branch + GITHUB_URL_STYLES_JSON), JSON_AUTHOR,
                                    new String());
                            final ResourceLocation resourceLocWithBranch = new ResourceLocation(
                                    resourceLoc.getNamespace(), resourceLoc.getPath() + "/" + branch);

                            // get styles
                            JsonUtils.parseFirstLayerAttribute(
                                    NetUtils.httpsGet(githubUrl + branch + GITHUB_URL_STYLES_JSON), JSON_STYLE + "s",
                                    new ArrayList<String>()).forEach(style -> {
                                        Log.getLogger().info(branch + " " + resourceLocWithBranch.toString() + " "
                                                + author + " " + style); // debug output here

                                        // create new Style and add it to styles
                                        final Style newStyle = JsonUtils.parseToStyle(
                                                NetUtils.httpsGet(githubUrl + branch + "/" + style
                                                        + GITHUB_URL_SINGLE_STYLE_JSON),
                                                style, resourceLocWithBranch, author);
                                        if (newStyle != null) {
                                            if (newStyle instanceof DefaultStyle) {
                                                newStyle.setLocalPath(
                                                        styles.getDownloadFolder() + "/" + newStyle.getId());
                                                Log.getLogger()
                                                        .info(styles.getDownloadFolder() + "/" + newStyle.getId());
                                            }
                                            styles.addStyle(newStyle);
                                        } else {
                                            Log.getLogger()
                                                    .warn("Failed to parse: " + style + ", from branch: " + branch);
                                        }
                                    });
                        });
            }
    ...
    /**
     * Fills registry into non-default styles, also fills bases, ---> must always
     * end with saving Styles <---
     */
    private static void doRegistry(final Styles styles) {
        ...
        for (int i = 0; i < styles.getStyles().size(); i++) {
            ...
            if (!(style instanceof DefaultStyle) && !style.getBase().isEmpty()) {
                final Style base = styles.getStyles().stream().filter(styleB -> styleB.getId().equals(style.getBase()))
                        ...
                        for (int k = 0; k < headBase.getTypes().size(); k++) {
                            ...
                            if (typeToCheck == null) {
                                ...
                                for (int l = 0; l < typeBase.getFiles().size(); l++) {
                                    ...
                                    final Trinuple<String, String, String> fileToCheck = typeToCheck
                                            .getFileById(fileBase.getFirst());
        ...
        final DefaultStyle defaultstyle = (DefaultStyle) styles.getStyles().stream()
                .filter(style -> style instanceof DefaultStyle).findFirst().orElse(null);
                ...
                for (int j = 0; j < defaultstyle.getRegistry().size(); j++) {
                    ...
                    if (headToCheck == null) {
                        ...
                        for (int k = 0; k < headToCheck.getTypes().size(); k++) {
                            ...
                            final DefaultType typeRegistry = (DefaultType) headRegistry
                                    .getTypeById(headRegistry.getRegistry().get(k));
                        ...
                        for (int k = 0; k < headRegistry.getRegistry().size(); k++) {
                            ...
                            if (typeToCheck == null) {
                                ...
                                for (int l = 0; l < typeRegistry.getRegistry().size(); l++) {
                                    ...
                                    final Trinuple<String, String, String> fileRegistry = typeRegistry
                                            .getFileById(file);
    ...
    /**
     * Update all downloaded styles which are also in styles Also downloads the
     * default style if already not downloaded
     */
    private static void updateDownloadableStyles(final Styles styles) {
        ...
        styles.getStyles().forEach(style -> {
            if (!style.getLocalPath().isEmpty() && style.getOrigin().getNamespace().equals(GITHUB_REPO_PREFIX)) {
                ...
                final Tuple<String, String> localLatestSha = latestCommits.stream()
                        .filter(localStyle -> localStyle.getFirst().equals(style.getId())).findFirst().orElse(null);
                final String styleOrigin = style.getOrigin().getPath();
                final String url0 = JsonUtils.parseSecondLayerAttribute(
                        NetUtils.httpsGet(
                                GITHUB_API_REPOS_PREFIX + styleOrigin.substring(0, styleOrigin.lastIndexOf("/"))
                                        + GITHUB_URL_REFS_HEADS + styleOrigin.substring(styleOrigin.lastIndexOf("/"))),
                        GITHUB_COMMITJSON_OBJECT, GITHUB_COMMITJSON_URL, new String());
                final String url1 = JsonUtils.parseSecondLayerAttribute(NetUtils.httpsGet(url0), GITHUB_COMMITJSON_TREE,
                        GITHUB_COMMITJSON_URL, new String());
                final String styleLatestSha = JsonUtils
                        .parseFirstLayerObjectArrayToTuple(NetUtils.httpsGet(url1), GITHUB_COMMITJSON_TREE,
                                GITHUB_COMMITJSON_PATH, GITHUB_COMMITJSON_SHA)
                        .stream().filter(styleId -> style.getId().equals(styleId.getFirst())).findFirst().get()
                        .getSecond();
                ...
                if (localLatestSha == null || !localLatestSha.getSecond().equals(styleLatestSha)) {
                    ...
                    if (localLatestSha == null) {
                        ...
                        latestCommits.set(latestCommits.indexOf(localLatestSha),
                                new Tuple<String, String>(style.getId(), styleLatestSha));
                    }
                }
            }
        });
    ...
    private static void downloadStyle(final Style style) {
        ...
        final ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors
                .newFixedThreadPool(Configurations.amountOfThreadsForDownloading);
            ...
            head.getTypes().forEach(type -> {
                ...
                type.getFiles().forEach(file -> {
                    if (file.getThird().equals(type.getOrigin())) {
                        ...
                        final String styleOrigin = style.getOrigin().getPath();
                        final String url = GITHUB_URL_PREFIX + styleOrigin.substring(0, styleOrigin.lastIndexOf("/"))
                                + GITHUB_URL_RAW_WORD + styleOrigin.substring(styleOrigin.lastIndexOf("/")) + "/"
                                + style.getId() + "/" + head.getId() + "/" + type.getId() + "/" + file.getFirst()
